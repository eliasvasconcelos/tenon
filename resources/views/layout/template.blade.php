<!doctype html>
<html lang="pt-BR">
<head>
    <meta charset="">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Tenon</title>
    <meta name="Description" content="">
    <meta name="author" content="Elias Vasconcelos">
    <meta keywords="">
    <!-- CSRF Token -->
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <link rel="shortcut icon" href="">
    <link rel="stylesheet" href="{{ asset('css/style.css') }}">
    <link rel="stylesheet" href="{{ asset('css/resolucao.css') }}">
    <link rel="stylesheet" href="{{ asset ('css/reset.css')}}">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
    <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,300i,400,400i,600,600i,700,700i,800,800i" rel="stylesheet">
    <script src="https://code.jquery.com/jquery-3.2.1.js"></script>
    <script src="{{ asset('js/valida.js') }}"></script>
    <script>
        document.addEventListener('DOMContentLoaded', function(){
            Typed.new('.element', {
                strings: [" pesquise aqui....", " ex: vaca nuvilha"],
                typeSpeed: 0
            });
        });
        $(document).ready(function() {
            var nav = $('.top_esq');
            $(window).scroll(function () {
                if ($(this).scrollTop() > 540) {
                    nav.addClass("fixo");
                } else {
                    nav.removeClass("fixo");
                }
            });
        });
        $(document).ready(function() {
            var nav = $('.cat_principal');
            $(window).scroll(function () {
                if ($(this).scrollTop() > 540) {
                    nav.addClass("cat_fixa");
                } else {
                    nav.removeClass("cat_fixa");
                }
            });
        });
    </script>
</head>
<body>
<div class="container">
    <div class="list"></div>
    <header class="topo">
        <a href ="{{ url('/') }}" title="Home">
            <div class="logo swing fade"><h1>Tenon - Tudo do meio Rural em um só local</h1></div>
        </a>
        <span class="saudacao">
            @guest
                Olá! <a href="{{ url('/') }}/login">Entre</a> ou <a href="{{ url('/') }}/register">cadastre-se</a>
                @else
                    Bem Vindo!  {{ Auth::user()->name }} {{ Auth::user()->sobre_nome }}
                    @endguest
                </span>
        <!-- MENU MOBILE -->
        <ul class="menu_mobile">
            <h1 class="fontzero">Menu Mobile - Tenon</h1>
            <li class="right text-right">
                @guest
                    <a title="Acesse nossa plataforma" class="botao visivel" href="{{ url('/') }}"><i class="fa fa-home fa-lg chat"></i> &nbsp; Home </a><span class="visivel">|</span>
                    <a title="Acesse nossa plataforma" class="botao" href="{{ url('/') }}/login"><i class="fa fa-user-circle-o  fa-lg user"></i> &nbsp; Minha Conta </a>
                    @else

                        @endguest
            </li>
        </ul>
        <!-- MENU FULL TOPO -->
        <ul class="menu">
            <h1 class="fontzero">Acesse o Menu Desktop</h1>
            <li class="right text-right">
                @guest
                    <a title="Acesse nossa plataforma" class="botao" href="{{ url('/') }}/chat"><i class="fa fa-comments fa-lg"></i> &nbsp; Chat </a>|
                    <a title="Acesse nossa plataforma" class="botao" href="{{ url('/') }}/login"><i class="fa fa-user-circle-o  fa-lg"></i> &nbsp; Minha Conta </a>
                    @else
                        <a title="Desconectar" class="botao" href="{{ route('logout') }}"><i class="fa fa-close  fa-lg"></i> &nbsp; Desconectar </a>
                        @endguest

            </li>
        </ul>

        <section class="intro swing">
            <a href="#"><h1>Encontre tudo</h1></a>
        </section>
        <p class="tagline">do comércio agropecuário em um só local...</p>
    </header>

    <section class="header_op">
        <h1 class="fontzero"> Pesquisar produtos na plataforma Tenon </h1>

        @guest
            <a href="{{ url('/') }}/login">
        <span class="anunciar2 swing">
            <i class="fa fa-bullhorn fa-lg"></i> Inserir anúncio
        </span>
            </a>
            @else
                <a href="{{ url('/') }}/meus_anuncios">

            <span class="ofertas swing">
                <i class="fa fa-user fa-lg"></i> Meus anúncios
            </span>

                </a>
                <a href="{{ url('/') }}/new">

        <span class="anunciar swing">
            <i class="fa fa-bullhorn fa-lg"></i> Inserir anúncio
        </span>


                </a>

                @endguest

                <div class="bg_busca">
                    <div class="busca">
                        <i class="fa fa-search fa-3x busca_icon"></i>
                        <form action="" method="get">
                            <input id="busca-cursor" type="text" class="header_pesquisa element">
                            <select class="categorias">
                                <option value="all" selected>O que é?</option>
                            </select>
                            <select name="id_estados" class="estados" id="id_estados">
                                <option value="all" selected>Qual local?
                                </option>
                                <option value="AC">Acre</option>
                                <option value="AL">Alagoas</option>
                                <option value="AP">Amapá</option>
                                <option value="AM">Amazonas</option>
                                <option value="BA">Bahia</option>
                                <option value="CE">Ceará</option>
                                <option value="DF">Distrito Federal</option>
                                <option value="ES">Espirito Santo</option>
                                <option value="GO">Goiás</option>
                                <option value="MA">Maranhão</option>
                                <option value="MT">Mato Grosso</option>
                                <option value="MS">Mato Grosso do Sul</option>
                                <option value="MG">Minas Gerais</option>
                                <option value="PA">Pará</option>
                                <option value="PB">Paraiba</option>
                                <option value="PR">Paraná</option>
                                <option value="PE">Pernambuco</option>
                                <option value="PI">Piauí</option>
                                <option value="RJ">Rio de Janeiro</option>
                                <option value="RN">Rio Grande do Norte</option>
                                <option value="RS">Rio Grande do Sul</option>
                                <option value="RO">Rondônia</option>
                                <option value="RR">Roraima</option>
                                <option value="SC">Santa Catarina</option>
                                <option value="SP">São Paulo</option>
                                <option value="SE">Sergipe</option>
                                <option value="TO">Tocantis</option>
                            </select>
                            <button name="botao" class="btn_busca"><i class="fa fa-search fa-lg b_pes"></i> pesquisar</button>
                        </form>
                    </div>
                </div>
    </section>
    <div class="bg_branco">
        <menu class="cat_principal">
            <div class="center">
                <h1 class="fontzero">Pesquise por Categorias</h1>
                <ul>
                    @forelse($categoria as $z)
                    <a href="" class="fade_m"><li><i class="fa fa-paw fa-2x"></i><p>{{$z->nome}}</p></li></a>
                    @empty
                        <li>Não há registro!</li>
                    @endforelse
                    <!--   <a href="" class="fade_m"><li><i class="fa fa-picture-o fa-2x"></i><p>Artesanato</p></li></a>
                    <a href="" class="fade_m"><li><i class="fa fa-graduation-cap fa-2x"></i><p>Cursos</p></li></a>
                    <a href="" class="fade_m"><li><i class="fa fa-map fa-2x"></i><p>Equitação</p></li></a>
                    <a href="" class="fade_m"><li><i class="fa fa-camera fa-2x"></i><p>Eventos</p></li></a>
                    <a href="" class="fade_m"><li><i class="fa fa-home fa-2x"></i><p>Imóveis</p></li></a>
                    <a href="" class="fade_m"><li><i class="fa fa-truck fa-2x"></i><p>Maquinário</p></li></a>
                    <a href="" class="fade_m"><li><i class="fa fa-leaf fa-2x"></i><p>Mudas/Ramos</p></li></a>
                    <a href="" class="fade_m all"><li><i class="fa fa-bars fa-2x"></i><p>All Categorias</p></li></a>
                              <li><a href="#" class="fade_m">Suprimentos</a></li>
                                    <li class="oculta right"><a href="?pg=resultado" class="fade_m"><i class="fa fa-lemon-o fa-2x"></i><p>Sementes</p>
                                    <li><a href="#" class="fade_m">Vestimentas</a></li>
                                    <li><a href="#" class="fade_m">Veículos</a></li>-->
                </ul>
            </div>
        </menu>
    </div>
</div>

@yield('conteudo')

<footer id="rodape">
    <div class="bg">
        <div class="center">
            <div class="left"><img src="img/Logo.png"></div>
            <div class="middle">&copy; 2017 - Tenon - Todos os direitos Reservados</div>
            <div class="right">

                Share<br /><br />
                <i class="fa fa-rss-square fa-2x rodape_icon"></i>
                <i class="fa fa-youtube-square fa-2x rodape_icon"></i>
                <i class="fa fa-facebook-square fa-2x rodape_icon"></i>
                <i class="fa fa-twitter-square fa-2x rodape_icon"></i>
            </div>
            <div class="empresa">Lorem ipsum dolor sit amet, consectetur adipiscing elit,incididunt ut labore et dolore magna aliqua. Ut enim ad quis nostrud exercitation ullamco laboris.</div>
            <div class="menu">
                <h1 class="fontzero">Links Úteis da Plataforma</h1>
                <ul>
                    <li><h3> Apoio?</h3></li>
                    <li> <a href="ajuda">Ajuda</a></li>
                    <li> <a href="protecao">Proteção</a></li>
                </ul>

                <ul>
                    <li><h3> Proteção</h3></li>
                    <li> <a href="regras">Regras</a></li>
                </ul>

                <ul>
                    <li><h3> Divulgue</h3></li>
                    <li> <a href="#">Segurança</a></li>
                </ul>

                <ul>
                    <li><h3> Sobre Nós</h3></li>
                    <li> <a href="#">Publicidade</a></li>
                    <li> <a href="#">Localização</a></li>
                </ul>
            </div>
        </div>
    </div>
</footer>
</body>
</html>
